...
import { tweetEmbedSrc } from '../tweet-embed'
...
describe('detectEmbed — Twitter/X', () => {
  ...
  it('builds a self-contained data url carrying the tweet id and theme', () => {
    const dataUrl = tweetEmbedSrc('20', 'https://x.com/jack/status/20', 'dark')

    expect(dataUrl.startsWith('data:text/html')).toBe(true)
    const html = decodeURIComponent(dataUrl.slice(dataUrl.indexOf(',') + 1))
    expect(html).toContain('data-tweet-id="20"')
